...
export default function GFX() {
    ...
    return (
        <div className="flex flex-col text-white w-4/5 h-3/5">
            <div className="flex flex-col items-center justify-center font-red-hat-display text-white text-[20px] tracking-[0.2rem] text-center py-4 flex-shrink-0">
                <p>(I may add more stuff but i'm too lazy rn lol)</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-8">
                    {gfxWork.map((work, index) => (
                        <div 
                            key={index} 
                            className={`font-red-hat-display flex flex-wrap items-start justify-center py-6 px-6 rounded-2xl tracking-[0.2rem] gap-6 transition-transform duration-300`}
                        ...
                        >
                            ...
                            <div className="flex-shrink-0">
                                ...
                                >
                                    <Image 
                                        ...
                                        height={180}
                                        quality={100}
                                        className="hover:scale-110 transition-transform duration-300 select-none"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex-1 min-w-[280px]">
                                <a 
                                    href={work.gfxUrls[0]} 
                                    className="text-[24px] font-bold underline hover:text-[#F7395B] transition duration-300 block mb-3"
                                    target="_blank"
                                ...
                                <hr className="my-4 rounded h-[3px] bg-white border-0" />
                                
                                <div className="space-y-2">
                                    {work.descs.map((desc, descIndex) => (
                                        <p key={descIndex} className="text-[14px]">
                                            {desc}
                                        </p>
                                    ))}
                                </div>
                                ...
                                {work.titles.length > 1 && (
                                    <div className="mt-4 space-y-1">
                                        {work.titles.slice(1).map((title, titleIndex) => (
                                            <p key={titleIndex} className="text-[12px] text-gray-400 italic">
                                                <a
                                ...
                                {work.images.length > 1 && (
                                    <div className="mt-3">
                                        <span 
                                            className="text-[12px] text-gray-500 bg-gray-800 px-2 py-1 rounded cursor-pointer hover:bg-gray-700 hover:text-gray-300 transition-all duration-200"
            ...
            <AnimatePresence>
                {selectedImage && selectedWork && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: 'rgba(25,22,23,0.869)' }}
                        onClick={closeModal}
                    >
                        <motion.div
                            initial={{ scale: 0.8 }}
                        ...
                        >
                            ...
                            <button
                                onClick={closeModal}
                                className="cursor-pointer absolute top-[-15px] right-[-15px] z-10 bg-black bg-opacity-50 hover:bg-opacity-75 text-white rounded-full w-8 h-8 flex items-center justify-center text-lg font-bold transition-all duration-200"
                            >
                                ×
                            </button>
                            ...
                            {selectedWork.images.length > 1 && (
                                <button
                                    onClick={nextImage}
                                    className="cursor-pointer absolute top-[-15px] left-[-15px] z-10 bg-black bg-opacity-50 hover:bg-opacity-75 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold transition-all duration-200"
                                >
                                    →
                                </button>
                            )}
                            ...
                            {/* Image counter */}
                            {selectedWork.images.length > 1 && (
                                <div className="absolute bottom-[-25px] left-1/2 transform -translate-x-1/2 bg-black bg-opacity-50 text-white px-3 py-1 rounded text-sm">
